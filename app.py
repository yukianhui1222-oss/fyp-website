        ...
        with nav_cols[0]:
            ...
            if st.button(exit_label, use_container_width=True, key="exit_quiz_action_btn"):
                ...
                if review_mode:
                    ...
                    # User exited prematurely before finishing - don't keep abandoned quiz in chat memory
                    st.session_state.last_completed_quiz = None
                st.session_state.quiz_data = None
...
def format_summary_for_sharing(summary_text, doc_fname, translation_text=None, result_lang=""):
    """Formats 100% complete summary text and optional translation for WhatsApp & Email."""
    if not summary_text:
        return ""
    
    def _clean_md_for_chat(text):
        ...
        for line in lines:
            ...
            if l.startswith('### '):
                ...
                formatted.append(l.replace('**', '*'))
        return '\n'.join(formatted).strip()

    summary_body = _clean_md_for_chat(summary_text)
    
    if translation_text and translation_text.strip():
        trans_body = _clean_md_for_chat(translation_text)
        lang_title = f"{result_lang.upper()} TRANSLATION" if result_lang else "TRANSLATION"
        return (
            f"📚 *DocuMind Pro - Study Summary & Translation*\n"
            ...
            f"==============================\n"
            f"{trans_body}\n\n"
            f"✨ _Generated with DocuMind Pro 🚀_"
        )
    else:
        return (
            f"📚 *DocuMind Pro - Study Summary*\n"
            f"📄 *Document:* {doc_fname}\n\n"
            f"{summary_body}\n\n"
            f"✨ _Generated with DocuMind Pro 🚀_"
        )
...
def render_export_and_share_popover(docx_data, md_data, results, summary_result, translation_result="", result_lang="", key_suffix=""):
    ...
    with st.popover("📤 Export Document", use_container_width=True, key=f"export_document_popover{key_suffix}"):
        ...
        st.download_button(
            ...
            data=docx_data,
            file_name=f"{doc_fname}_Summary.docx",
            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            use_container_width=True,
            key=f"export_word_btn{key_suffix}"
        )
        st.download_button(
            label="📝 Export as Markdown (.md)",
            data=md_data,
            file_name=f"{doc_fname}_Summary.md",
            mime="text/markdown",
            use_container_width=True,
        ...
        wa_text = format_summary_for_sharing(summary_result, doc_fname, translation_result, result_lang)
        wa_url = f"https://api.whatsapp.com/send?text={urllib.parse.quote(wa_text)}"
        
        mail_subject = f"[DocuMind Pro] Study Summary: {doc_fname}"
        if translation_result and translation_result.strip():
            lang_title = f"{result_lang.upper()} TRANSLATION" if result_lang else "TRANSLATION"
            full_mail_body = (
                f"Hi,\n\nHere is the study summary and {result_lang} translation generated by DocuMind Pro for '{doc_fname}':\n\n"
                f"========================================\n"
                f"✨ ORIGINAL SUMMARY\n"
                f"========================================\n"
                f"{summary_result.strip() if summary_result else ''}\n\n"
                f"========================================\n"
                f"🌐 {lang_title}\n"
                f"========================================\n"
                f"{translation_result.strip()}\n\n"
                f"Best regards,\nDocuMind Pro 🚀"
            )
        else:
            full_mail_body = (
        ...
        # Gmail Web Compose URL (100% reliable on all browsers)
        gmail_url = f"https://mail.google.com/mail/?view=cm&fs=1&su={urllib.parse.quote(mail_subject)}&body={urllib.parse.quote(full_mail_body)}"
        # Safe mailto URL (trimmed to 1200 chars to prevent Windows 2048-char crash)
        safe_mailto_body = full_mail_body if len(full_mail_body) < 1200 else full_mail_body[:1150] + "...\n\n(Full summary & translation copied to clipboard!)"
        safe_mailto_url = f"mailto:?subject={urllib.parse.quote(mail_subject)}&body={urllib.parse.quote(safe_mailto_body)}"
        
        escaped_full_text = json.dumps(wa_text)
        escaped_mail_body = json.dumps(full_mail_body)
        ...
        share_html = f"""
            ...
            <div style="display: flex; flex-direction: column; width: 100%; padding-top: 2px;">
                <button id="{btn_id_wa}" class="dm-share-btn dm-share-wa">
                    ...
                    <span>Share via WhatsApp (Summary + Translation)</span>
                </button>
                <button id="{btn_id_mail}" class="dm-share-btn dm-share-mail">
                    <span style="font-size: 1rem;">✉️</span>
                    <span>Share via Email (Summary + Translation)</span>
                </button>
            </div>
            <script>
                ...
                document.getElementById('{btn_id_mail}').onclick = function() {{
                    ...
                    if (!gWin || gWin.closed || typeof gWin.closed == 'undefined') {{
                        // Fallback to safe mailto
                        window.location.href = "{safe_mailto_url}";
                    }}
                    btn.innerHTML = '<span>✅</span> Copied & Email Opened!';
                    setTimeout(() => {{ btn.innerHTML = orig; }}, 3000);
                }};
            </script>
        """
    ...
    if 'ocr_results' in st.session_state and not st.session_state.is_processing:
        ...
        if is_saved and st.session_state.get(rename_key, False):
            ...
            with export_col_r:
                ...
                render_export_and_share_popover(docx_data, md_data, results, summary_result, translation_result=translation_result, result_lang=result_lang, key_suffix="_r")
        else:
            ...
            with pencil_col:
                if is_saved:
                    if st.button("✏️", key=f"rename_btn_{doc_id}", help=f"Rename: {filename}"):
                        ...
                        st.rerun()
            with export_col:
                render_export_and_share_popover(docx_data, md_data, results, summary_result, translation_result=translation_result, result_lang=result_lang, key_suffix="_main")
        ...
        with main_col:
            ...
            with tab1:
                ...
                doc_fname = results.get('filename', 'DocuMind')
                wa_text_tab = format_summary_for_sharing(summary_result, doc_fname, translation_result, result_lang)
                wa_url_tab = f"https://api.whatsapp.com/send?text={urllib.parse.quote(wa_text_tab)}"

                mail_subject_tab = f"[DocuMind Pro] Study Summary: {doc_fname}"
                if translation_result and translation_result.strip():
                    lang_title_tab = f"{result_lang.upper()} TRANSLATION" if result_lang else "TRANSLATION"
                    full_mail_body_tab = (
                        f"Hi,\n\nHere is the study summary and {result_lang} translation generated by DocuMind Pro for '{doc_fname}':\n\n"
                        f"========================================\n"
                        f"✨ ORIGINAL SUMMARY\n"
                ...
                gmail_url_tab = f"https://mail.google.com/mail/?view=cm&fs=1&su={urllib.parse.quote(mail_subject_tab)}&body={urllib.parse.quote(full_mail_body_tab)}"
                # Safe mailto URL
                safe_mailto_body_tab = full_mail_body_tab if len(full_mail_body_tab) < 1200 else full_mail_body_tab[:1150] + "...\n\n(Full summary & translation copied to clipboard!)"
                safe_mailto_url_tab = f"mailto:?subject={urllib.parse.quote(mail_subject_tab)}&body={urllib.parse.quote(safe_mailto_body_tab)}"

                st.markdown("<div style='margin-top: 14px;'></div>", unsafe_allow_html=True)
                escaped_full_wa_tab = json.dumps(wa_text_tab)
                escaped_full_mail_tab = json.dumps(full_mail_body_tab)
                ...
                tab_share_html = f"""
                    ...
                    <div class="dm-tab-bar">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <span style="font-size: 0.86rem; font-weight: 700; color: #334155;">Share Summary & Translation:</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <script>
                        ...
                        document.getElementById('{btn_tab_mail}').onclick = function() {{
                            ...
                            if (!gWin || gWin.closed || typeof gWin.closed == 'undefined') {{
                                window.location.href = "{safe_mailto_url_tab}";
                            }}
                            btn.innerHTML = '<span>✅</span> Copied & Email Opened!';
                            setTimeout(() => {{ btn.innerHTML = orig; }}, 3000);
                        }};
            ...
            with tab2:
                ...
                if translation_result:
                    ...
                    st.markdown("<div style='margin-top: 14px;'></div>", unsafe_allow_html=True)
                    btn_tab2_wa = f"tab2-share-wa-{abs(hash(doc_fname + 'tab2_wa'))}"
                    btn_tab2_mail = f"tab2-share-mail-{abs(hash(doc_fname + 'tab2_mail'))}"
                    tab2_share_html = f"""
                        <meta name="viewport" content="width=device-width, initial-scale=1.0">
                        <style>
                            html, body {{
                        ...
                        <div class="dm-tab-bar">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <button id="{btn_tab2_wa}" class="dm-btn dm-btn-wa">
                                    <span>💬</span> WhatsApp
                                </button>
                                <button id="{btn_tab2_mail}" class="dm-btn dm-btn-mail">
                                    <span>✉️</span> Email
                                </button>
                            </div>
                        </div>
                        <script>
                            document.getElementById('{btn_tab2_wa}').onclick = function() {{
                                const btn = this;
                                const orig = btn.innerHTML;
                                navigator.clipboard.writeText({escaped_full_wa_tab}).catch(() => {{}});
                                window.open("{wa_url_tab}", "_blank");
                            ...
                            document.getElementById('{btn_tab2_mail}').onclick = function() {{
                                const btn = this;
                                const orig = btn.innerHTML;
                                navigator.clipboard.writeText({escaped_full_mail_tab}).catch(() => {{}});
                    ...
                    """
                    components.html(tab2_share_html, height=56)
                else:
