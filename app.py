...
def render_export_and_share_popover(docx_data, md_data, results, summary_result, key_suffix=""):
    ...
    with st.popover("📤 Export Document", use_container_width=True, key=f"export_document_popover{key_suffix}"):
        ...
        escaped_mail_body = json.dumps(mail_body)
    ...
    if 'ocr_results' in st.session_state and not st.session_state.is_processing:
        ...
        with main_col:
            ...
            with tab1:
                ...
                mail_body_tab = f"Hi,\n\nHere is the study summary generated by DocuMind Pro for '{doc_fname}':\n\n----------------------------------------\n{summary_result.strip() if summary_result else ''}\n----------------------------------------\n\nBest regards,\nDocuMind Pro 🚀"
                mail_url_tab = f"mailto:?subject={urllib.parse.quote(mail_subject_tab)}&body={urllib.parse.quote(mail_body_tab)}"

                st.markdown("<div style='margin-top: 14px;'></div>", unsafe_allow_html=True)
                escaped_full_wa_tab = json.dumps(wa_text_tab)
                escaped_full_mail_tab = json.dumps(mail_body_tab)
                ...
                tab_share_html = f"""
                    ...
                    <script>
                        ...
                        document.getElementById('{btn_tab_mail}').onclick = function() {{
                            ...
                            window.open("{mail_url_tab}", "_blank");
                            btn.innerHTML = '<span>✅</span> Copied & Email Opened!';
                            setTimeout(() => {{ btn.innerHTML = orig; }}, 3000);
                        }};
                    </script>
