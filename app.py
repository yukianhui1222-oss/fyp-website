...
def format_summary_for_sharing(summary_text, doc_fname):
    """Formats 100% complete summary text without any truncation for WhatsApp & Email."""
    if not summary_text:
        return ""
    ...
    return f"📚 *DocuMind Pro - Study Summary*\n📄 *Document:* {doc_fname}\n\n{body}\n\n✨ _Generated with DocuMind Pro 🚀_"
...
def render_export_and_share_popover(docx_data, md_data, results, summary_result, key_suffix=""):
    ...
    with st.popover("📤 Export Document", use_container_width=True, key=f"export_document_popover{key_suffix}"):
        ...
        st.markdown("<div style='font-size: 0.86rem; font-weight: 700; color: #1e293b; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;'><span>⚡</span> Quick Share Full Summary</div>", unsafe_allow_html=True)
        
        # Format 100% full, untruncated summary
        wa_text = format_summary_for_sharing(summary_result, doc_fname)
        wa_url = f"https://api.whatsapp.com/send?text={urllib.parse.quote(wa_text)}"
        
        mail_subject = f"[DocuMind Pro] Study Summary: {doc_fname}"
        full_mail_body = f"Hi,\n\nHere is the study summary generated by DocuMind Pro for '{doc_fname}':\n\n----------------------------------------\n{summary_result.strip() if summary_result else ''}\n----------------------------------------\n\nBest regards,\nDocuMind Pro 🚀"
        ...
        share_html = f"""
            ...
            <div style="display: flex; flex-direction: column; width: 100%; padding-top: 2px;">
                <button id="{btn_id_wa}" class="dm-share-btn dm-share-wa">
                    ...
                    <span>Share via WhatsApp (Full Text)</span>
                </button>
                <button id="{btn_id_mail}" class="dm-share-btn dm-share-mail">
                    <span style="font-size: 1rem;">✉️</span>
                    <span>Share via Email (Full Text)</span>
                </button>
            </div>
            <script>
    ...
    if 'ocr_results' in st.session_state and not st.session_state.is_processing:
        ...
        if is_saved and st.session_state.get(rename_key, False):
            ...
            with export_col_r:
                ...
                render_export_and_share_popover(docx_data, md_data, results, summary_result, key_suffix="_r")
        else:
            ...
            with pencil_col:
                if is_saved:
                    if st.button("✏️", key=f"rename_btn_{doc_id}", help=f"Rename: {filename}"):
                        ...
                        st.rerun()
            with export_col:
                render_export_and_share_popover(docx_data, md_data, results, summary_result, key_suffix="_main")
        ...
        with main_col:
            ...
            with tab1:
                ...
                doc_fname = results.get('filename', 'DocuMind')
                wa_text_tab = format_summary_for_sharing(summary_result, doc_fname)
                ...
                tab_share_html = f"""
                    ...
                    <div class="dm-tab-bar">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <span style="font-size: 0.86rem; font-weight: 700; color: #334155;">Share Full Summary:</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <button id="{btn_tab_wa}" class="dm-btn dm-btn-wa">
                                <span>💬</span> WhatsApp (Full Text)
                            </button>
                            <button id="{btn_tab_mail}" class="dm-btn dm-btn-mail">
                                <span>✉️</span> Email (Full Text)
                            </button>
                        </div>
                    </div>
                    <script>
